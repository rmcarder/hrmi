...
setwd("C:/Users/rcarder/Documents/Dev/hrmi/Shapefiles")

#PC Paths
shapefile_path<-"C:/Users/rcarder/Documents/Dev/hrmi/Shapefiles/countries_raw"
serf_path<-"C:/Users/rcarder/Documents/Dev/hrmi/Shapefiles/SERF2017.csv"
output_path<-"C:/Users/rcarder/Documents/Dev/hrmi/Shapefiles/Output"

shapefile <- readOGR(dsn = shapefile_path, layer = "ne_10m_admin_0_countries", stringsAsFactors = F)

# getting data as data frame
df <- shapefile@data
write.csv(df, file = "ShapefileData.csv")

# Disaster likelyhood by fips. Available if you have accest to proprietary data
# replace /Users/jakesnyder/DisasterVulnerability/ with the path to the data on your machine
serf_data <- read.csv(serf_path,stringsAsFactors = F)

#remove extra columns
#hazard_data<-hazard_data[,1:12]

# remove NA's from hazard data
serf_data <- na.omit(serf_data)
...
merge_data <- merge(shapefile, serf_data, by.x="ADM0_A3", by.y="Country.Code", all.x=T)
#merge_data <- merge(merge_data, ccusa, by = "FIPS", all.x=T)
dfmerge <- merge_data@data
#write.csv(dfmerge, file = "ShapefileData.csv")
...
merge_data@data <- dfmerge

# replace /Users/jakesnyder/DisasterVulnerability/ with the path to your desired directory
writeOGR(merge_data, dsn=output_path, layer="countries", driver="ESRI Shapefile", overwrite_layer = T)
?writeOGR
